...
hidden_output = torch.randn(batch_size, seq_len, intermediate_size).npu()

# Create the last linear layer, mapping hidden layer size to vocabulary size
# linear_layer = nn.Linear(intermediate_size, vocab_size).npu()
...
weight_matrix = torch.randn(intermediate_size, vocab_size).npu()
